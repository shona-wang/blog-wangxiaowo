<%@ Page Language="VB" AutoEventWireup="false" CodeFile="column.aspx.vb" Inherits="cms_column_column" %>
        ...
        <div showcollapsebutton="true">
            <div class="mini-fit">
                <div id="grid1" class="mini-datagrid" style="width: 100%; height: 100%;" borderstyle="border:0;"
                    url="../service/column.aspx?method=query" showfilterrow="false" allowcellselect="true"
                    ...
                    <div property="columns">
                        ...
                        <div field="url" width="175" headeralign="center" renderer="url_renderer" allowsort="true">
                            链接地址
                        </div>
                        <div field="pid" width="120" headeralign="center" allowsort="true" visible="false">
                            父节点代码
                        </div>
...
//        });
    ...
    function addRow() {
        ...
        mini.open({
            url: "../cms/column/add-column.aspx",
            title: "新增栏目", width: 500, height: 300,
            onload: function() {
                var iframe = this.getIFrameEl();
                var data = "";
                data = { action: "add", pid: node.id };
        ...
        if (rows.length > 0) {
            ...
            function(action) {
                if (action == "ok") {
                    ...
                    $.ajax({
                        url: "../service/column.aspx?method=deletes&ids=" + id,
                        success: function(text) {
                            if (text != "-1") {
                                grid.reload();
    ...
    function treereload() {
        tree.load("../service/column.aspx?method=query");
    }

    function url_renderer(e) {
        var record = e.record;
        if (record.url == null) {
            return "";
        }
        else {
            var s = ' <a class="" href="javascript:url_CK(\'' + record.url + '\')">' + record.url + '</a>';
            return s;
        }
    }


    function url_CK(record) {
        if (record != "") {
            window.open(record);
        }
    }
