<%@ Page Language="C#" AutoEventWireup="true" CodeFile="tp_index.aspx.cs" Inherits="WeiXinAspx_WeiXin_MessageReplySetUp_tp_tp_index" %>
...
<%--    <span class="separator"></span><a class="mini-button" iconcls="icon-print" onclick="print()" plain="true">打印预览</a>--%>
  ...
  <div class="mini-fit">
    <div id="WeiXin_MessageReplySetUpdatagrid" class="mini-datagrid" style="height: 98%;" url="../WeiXin_MessageReplySetUpData.aspx?method=WeiXin_MessageReplySetUp_PageJson_SelectWhere" idfield="ID" allowresize="true" pagesize="100" allowcellselect="true" multiselect="true">
      <div property="columns">
        ...
        <div field="MessagContentAndId"   headeralign="center" allowsort="true"  >
        图片
        </div>
 ...
 <script type="text/javascript">
    ...
    WeiXin_MessageReplySetUpgrid.load({LX:'TP'});
    WeiXin_MessageReplySetUpgrid.on("drawcell", function(e) {
        var record = e.record,
         column = e.column,
         field = e.field,
         value = e.value;
        if (column.field == "MessagContentAndId") {
            e.cellHtml = "<img src='../../../UploadFiles/" + value + "' style=' width:260px; height:100px;'/>";
        }
    });
        //添加
     function add() {
          mini.open({
          url: "../WeiXinAspx/WeiXin_MessageReplySetUp/tp/Edit.aspx",//此处注意修改
          title: "新增-图片信息", width: 400, height: 270,
          onload: function() {
      ...
      if (row) {
          mini.open({
              url:"../WeiXinAspx/WeiXin_MessageReplySetUp/tp/Edit.aspx",
              title: "修改-图片信息", width: 400, height: 270,
              onload: function() {
                  var iframe = this.getIFrameEl();
     ...
     if (rows.length > 0) {
         ...
         function(action) {
             if (action =="ok") {
                 ...
                 $.ajax({
                 url: "../WeiXin_MessageReplySetUpData.aspx?method=WeiXin_MessageReplySetUp_Deletes&&DJBH=" + id, //此处需要手动调整
