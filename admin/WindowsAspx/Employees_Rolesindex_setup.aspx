<%@ Page Language="C#" AutoEventWireup="true" CodeFile="Employees_Rolesindex_setup.aspx.cs" Inherits="WindowsAspx_Employees_Rolesindex_setup" %>

<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">

<html xmlns="http://www.w3.org/1999/xhtml">
<head runat="server">
    <title>组织机构权限设置页面</title>
...
<body>
     <div class="mini-toolbar">
          <a class="mini-button" iconcls="icon-save" onclick="SysGroupPowerOrganizationSave()"
...
<script type="text/javascript">
    ...
    $(window).load(function() {
        ...
        mini.get("DWID").setValue(Request["DWID"]);
        treegriedload(mini.get("DWID").getValue());
    })
    var showAllSelect = true;
    function treegriedload(DWID) {
        treegrid.load("AspxDate.aspx?method=SysGroupPowerOrganizationSelect&DWID=" + DWID);
    }
    ...
    function SysGroupPowerOrganizationSave() {
        //debugger;
        if (mini.get("DWID").getValue() != "") {
            var data = treegrid.getData();
            var json = mini.encode(data);
//            var treedata = tree.getValue(true);
//            var treejson = mini.encode(treedata);
            $.ajax({
            url: "AspxDate.aspx?method=SysGroupPowerOrganizationSave&DWID=" + mini.get("DWID").getValue(),
                data: {MenuJson: json },
                type: "post",
