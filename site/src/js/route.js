...
//触发跳转
    Route.go = function (hash) {
    	var param = Route.params();
    	param = param?param+"/":param;
        location.hash = "#" + Route.params() +hash;
    };
    /**
      * 获取/设置前置参数
      */
    Route.params = function (v) {
        var hash = location.hash,param="",flg=true;
        if(hash){
        	param = hash.substr(0, hash.indexOf("/"));
        	if (param){
        		param = param.replace("#", "");
        		for(var k in routes){
	        		if(k.indexOf(param)>=0){//正常路由，不需要进行参数处理
	        			flg=false;
	        		}
	        	}
	        	if(!flg){
	        ...
	        if (v) {
	            hash = "#" + v+ "/" + hash.substr(hash.indexOf("/"));
	        } else {
	            return param || "";
	        }
        }else{
